 ...
 */
const updateResponseHeadersInDevFactory = () => {
  const doNothing = () => {};
  ...
  if (typeof config.kit?.vite === 'object' && config.server?.headers) {
    // Extract the headers...
    const headerArray = Object.entries(config.server?.headers)
      .filter(([k, v]) => v)
      .map(([k, v]) => [k, v?.toString() || '']);

    // Generate a fn that updates the response headers.
    const updateHeaders = (r) => {
